...
public class GameController implements Observer {
    ...
    void checkMine(int row, int col) {
        ...
        if( adjacentMines > 0 )
            ...
            for( int r = row - 1; r <= row + 1; r++ )     // a 3x3 sub-grid of tiles adjacent to current tile (row, col)
        ...
        if(tilesClicked == (size * size) - mineCount) {
            gameOver = true;
            view.setStatusPanel("You won! Mines cleared!");
        }
    }
}
